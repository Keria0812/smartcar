...
setup_client (void)
{
    ...
    if (Srv == NULL) {
        ...
        error_and_exit("Could not connect, the Apphost is %s, the AppUser is %s, AppClass is %s .\n", AppHost, AppUser , AppClass);
    }
...
void concatenateStrings(const char* str1, const char* str2, char* result) {
    ...
    strcat(result, str2);  // 将str2追加到result末尾
}
/******************************************************************************/

static void
demo_StartStop ( const char *TestRunCase)
{
    // char *result;
    // const char* testcase = 
    // printf("Run 1000 seconds of Hockenheim\n");
    // char res_case[strlen("LoadTestRun myTest/") + strlen(TestRunCase) + 1]
    char result[2000];
    strcpy(result, "LoadTestRun myTest/");
    strcpy(result + 19, TestRunCase);

    strcat(result," 1");
    printf("%s \n",result );
    GuiCmd_Eval(Srv, result);
    GuiCmd_Eval(Srv, "StartSim");
    GuiCmd_Eval(Srv, "WaitForStatus running");
    // GuiCmd_Eval(Srv, "WaitForTime 1000");
...
{
    double Time = 0, Car_v = 0, Car_Distance = 0;
    int DM_GearNo = 0, i;
    float Dummy;
...
{
    const char *example = "";
    const char *TestRunCase = "";

    // char* TestRunCase[160];
	...
	demo_DVARead();
    }  else if (strcasecmp(example, "Stop") == 0) {
    demo_GUIStop();
    }else {
