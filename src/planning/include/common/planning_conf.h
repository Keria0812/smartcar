...
public:
    ...
    int32_t FLAGS_planning_loop_rate = 10;

    uint64_t FLAGS_trajectory_stitching_preserved_length = 20;


    double FLAGS_trajectory_time_resolution = 0.1;      // Trajectory time resolution in planning
    double FLAGS_weight_target_speed = 1.0;             // Weight of target speed cost
    double FLAGS_weight_dist_travelled = 10.0;          // Weight of travelled distance cost
    double FLAGS_longitudinal_jerk_lower_bound = -4.0;  // The lower bound of longitudinal jerk.
    double FLAGS_longitudinal_jerk_upper_bound = 2.0;   // The upper bound of longitudinal jerk
    double FLAGS_lon_collision_cost_std = 0.5;          // The standard deviation of longitudinal collision cost function
    double FLAGS_lon_collision_yield_buffer = 1.0;      // Longitudinal collision buffer for yield
    double FLAGS_lon_collision_overtake_buffer = 5.0;   // Longitudinal collision buffer for overtake

    double FLAGS_weight_lon_objective = 10;
    double FLAGS_weight_lon_jerk = 1.0;
    double FLAGS_weight_lon_collision = 5.0;
    double FLAGS_weight_centripetal_acceleration = 1.5;
    double FLAGS_weight_lat_offset = 2.0;
    double FLAGS_weight_lat_comfort = 10.0;

    double FLAGS_speed_lon_decision_horizon = 60;   // Longitudinal horizon for speed decision making (meter)
    double FLAGS_trajectory_space_resolution = 1.0;  // Trajectory space resolution in planning
    double FLAGS_lat_offset_bound = 3.0;             // The bound of lateral offset
    double FLAGS_weight_opposite_side_offset = 10;   // Weight of opposite side lateral offset cost
    double FLAGS_weight_same_side_offset = 1.0;      // Weight of same side lateral offset cost

    double FLAGS_speed_lower_bound = -0.1;                      // The lowest speed allowed
    double FLAGS_speed_upper_bound = 20;                        // The highest speed allowed
    double FLAGS_longitudinal_acceleration_lower_bound = -4.5;  // The lowest longitudinal acceleration allowed
    double FLAGS_longitudinal_acceleration_upper_bound = 4;     // The highest longitudinal acceleration allowed
    double FLAGS_comfort_acceleration_factor = 0.5;             // Factor for comfort acceleration
    double FLAGS_Max_curvature = 0.1;                           //轨迹的最大曲率
    double FLAGS_kappa_bound = 0.20;
    double FLAGS_dkappa_bound = 0.02;
    double FLAGS_lateral_acceleration_bound = 4.0;
    double FLAGS_lateral_jerk_bound = 4.0;

    //纵向规划
    double FLAGS_trajectory_time_length = 8.0;     // 最大预测时间 [s]，8s
    double FLAGS_polynomial_minimal_param = 0.01;  // Minimal time parameter in polynomials
    double FLAGS_num_velocity_sample = 6;          // The number of velocity samples in end condition sampler
    double FLAGS_min_velocity_sample_gap = 1.0;    // Minimal sampling gap for velocity

    //二次规划
    bool FLAGS_enable_osqp_debug = false;
    double FLAGS_weight_lateral_offset = 1.0;
    double FLAGS_weight_lateral_derivative = 500.0;
    double FLAGS_weight_lateral_second_order_derivative = 1000.0;
    double FLAGS_lateral_third_order_derivative_max = 0.1;
    double FLAGS_weight_lateral_obstacle_distance = 0.0;
    double FLAGS_default_delta_s_lateral_optimization = 1.0;
    double FLAGS_max_s_lateral_optimization = 50.0;
    double FLAGS_bound_buffer = 0.1;
    double FLAGS_numerical_epsilon = 1e-6;  //横向优化的边界缓冲
    double FLAGS_nudge_buffer = 0.2;        //用于横向优化的微移缓冲
    double kSampleDistance = 0.25;
    double FLAGS_lon_collision_buffer = 1.0;
    double FLAGS_lat_collision_buffer = 0.4;
    ...
    double FLAGS_default_lon_buffer = 5.0;
    double FLAGS_time_min_density = 1.0;
    double FLAGS_num_sample_follow_per_timestamp = 3;

    double FLAGS_vehicle_width = 2.16;   //车的宽度
    double FLAGS_vehicle_length = 4.97;  //车的长度
    ...
    double default_active_set_eps_iter_ref;  //"qpOases wrapper early termination tolerance for iterative refinement"//
                                             //NOLINT
    ...
    double default_qp_smoothing_eps_iter_ref;   //"qpOases wrapper early termination tolerance for iterative refinement"
                                                //// NOLINT
    ...
    double
        FLAGS_trajectory_time_max_interval;  //"(seconds) Trajectory time interval when publish. The is the max value."
