...
std::vector<std::pair<double, double>> PathTimeGraph::GetPathBlockingIntervals(const double t)
{
  ...
  for (auto &pt_obstacle : path_time_obstacles_)
  {
    ...
    double left_lower_s = pt_obstacle.bottom_left_point_.s();
    double left_lower_t = pt_obstacle.bottom_left_point_.t();
    double left_upper_s = pt_obstacle.upper_left_point_.s();
    double left_upper_t = pt_obstacle.upper_left_point_.t();

    double right_lower_s = pt_obstacle.bottom_right_point_.s();
    double right_lower_t = pt_obstacle.bottom_right_point_.t();
    double right_upper_s = pt_obstacle.upper_right_point_.s();
    double right_upper_t = pt_obstacle.upper_right_point_.t();
  ...
  //                           path_time_obstacle_map_[obstacle->obstacle_id].bottom_right_point_.t());
    ...
    // std::cout << "maxt:" << left_lower_s << std::endl;
    // std::cout << "mint:" << left_upper_s << std::endl;
    // std::cout << "max_s:" << right_lower_s << std::endl;
    // std::cout << "min_s:" << right_upper_s << std::endl;

    // std::cout << "maxt:" << left_lower_t << std::endl;
    // std::cout << "mint:" << left_upper_t << std::endl;
    // std::cout << "max_s:" << right_lower_t << std::endl;
    // std::cout << "min_s:" << right_upper_t << std::endl;
    // std::cout << "///////////////////////////////" << std::endl;

    if (t > right_upper_t || t < left_lower_t || t > right_lower_t || t < left_upper_t)
    {
      continue;
    }
    // lerp 线性插值 接收三个数据 a,b,t 该函数返回a,b之间以t 为百分比的值
    double s_lower = common::math::lerp(left_lower_s, left_lower_t, right_lower_s, right_lower_t, t);

    double s_upper = common::math::lerp(left_upper_s, left_upper_t, right_upper_s, right_upper_t, t);
...
{
  // FLAGS_prediction_total_time其实就是PRediction模块中障碍物轨迹预测总时间，8s。
  // eval_time_interval其实就是Prediction模块中障碍物两个预测轨迹点的时间差，0.1s。
  //所以一共差不多80个位置点。最后对参考线上的每个障碍物在每个时间点设定位置标定框。
                                      ...
                                      const std::vector<ReferencePoint> &discretized_ref_points)
{
  ...
  if (sl_boundary.start_s_ > path_range_.second ||
      ...
      sl_boundary.end_l_ < -right_width) //这里暂时简化为起始点和终点都在车道以内，如果后续添加预测，记得修改这里
  {
                                       ...
                                       const std::vector<ReferencePoint> &discretized_ref_points)
{
  ...
  // std::cout << "path_range_first: " << path_range_.first << std::endl;
  // std::cout << "path_range_second: " << path_range_.second << std::endl;
  ...
  while (relative_time < time_range_.second) // 8s
  {
    ...
    // std::cout << box.center_x() << " " << box.center_y() << endl;
    // std::cout << discretized_ref_points[0].x_ << " " << discretized_ref_points[0].y_ << endl;
    ...
    // std::cout << "start_s: " << sl_boundary.start_s_ << std::endl;
    // std::cout << "end_s: " << sl_boundary.end_s_ << std::endl;
    // std::cout << "start_l: " << sl_boundary.start_l_ << std::endl;
    // std::cout << "end_l: " << sl_boundary.end_l_ << std::endl;
    // std::cout << "path_range_first: " << path_range_.first << std::endl;
    // std::cout << "path_range_second: " << path_range_.second << std::endl;
    // std::cout << "left_width: " << left_width << std::endl;
    // std::cout << "right_width: " << right_width << std::endl;
    ...
    const double t_min_density) const
{
  ...
  for (size_t i = 0; i <= num_sections; ++i)
  {
    ...
    double s;
    if (bounds.size() == 0) {
      s = common::math::lerp(s0, t0, s1, t1, t) + s_dist;
    } else {
      auto it_lower = std::lower_bound(bounds.begin(), bounds.end(), t, comp);
      s = bounds[it_lower - bounds.begin()].second;
    }
...
ST_Boundary PathTimeGraph::getACCTargetST(double ego_s) const {
  ST_Boundary acc_target;
  bool first = true;
  printf("%ld \n", path_time_obstacle_map_.size());
  for (const auto obs : path_time_obstacle_map_) {
    printf("{ %f %f}\n" ,obs.second.bottom_left_point_.s(), obs.second.upper_left_point_.s());
    if (first) {
      if (obs.second.bottom_left_point_.s() > ego_s) {
        acc_target = obs.second;
        first = false;
        continue;
      } else {
        continue;
    ...
    if (obs.second.bottom_left_point_.s() < acc_target.bottom_left_point_.s() &&obs.second.bottom_left_point_.s() > ego_s) {
      acc_target = obs.second;
    }
  }
  if (first) acc_target.obstacle_id = "-1";//错误信息
  return acc_target;
}
bool PathTimeGraph::getSLGraph(std::vector<DiscretizedTrajectory> &cur_sl_graph) {
  for (const auto obs : path_time_obstacle_map_) {
    // obs.second.
  }
};