...
std::vector<Condition> EndConditionSampler::SampleLatEndConditions() const
{
  ...
  // std::array<double, 15> end_d_candidates = {-1.5, -1.25 , -1, -0.75, -0.5,-0.25, 0,0.25, 0.5, 0.75, 1 , 1.25 ,1.5};
  // std::array<double, 7> end_d_candidates = {-1, -0.75, -0.5, 0, 0.5, 0.75, 1};
  // std::array<double, 5> end_d_candidates = {-1.0, -0.5, 0, 0.5, 1.0};
  std::array<double, 1> end_d_candidates = {0};

  // std::array<double, 5> end_d_candidates = {-0.75, -0.5, 0, 0.5, 0.75};

  std::array<double, 5> end_s_candidates = { 20.0, 40.0, 60.0, 80, 100};//直接意义是多长时间到达此横向位置
  // std::array<double, 6> end_s_candidates = {10.0, 20.0, 40.0, 80.0, 100, 120};//直接意义是多长时间到达此横向位置

  // std::array<double, 12> end_s_candidates = {10.0, 20.0, 30.0, 40.0, 50,60,70,80,90,100,110,120};

  // std::array<double, 5> end_s_candidates = {10, 15, 20,30, 40}; //低速40
