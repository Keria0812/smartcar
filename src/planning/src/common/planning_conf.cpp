...
Status PlanningConf::ReadControlConf(const string plan_conf_file_name) {
    ROS_INFO_STREAM("plan_conf_file_name: " << plan_conf_file_name);

    YAML::Node config = YAML::LoadFile(plan_conf_file_name.c_str());

    //yaml读取的参数，.yaml中一定要存在，不然报错
    //参考系的定义
    ...
    FLAGS_enable_force_pull_over_open_space_parking_test =
        config["FLAGS_enable_force_pull_over_open_space_parking_test"].as<bool>();
    ...
    FLAGS_Plan4NGSIM_ = config["FLAGS_Plan4NGSIM"].as<bool>();
...
void PlanningConf::printData() {
    ROS_INFO_STREAM("Frame_id is " << Frame_id);
    std::cout << "\n";

    ROS_INFO("FLAGS_trajectory_time_resolution : %f", FLAGS_trajectory_time_resolution);
    ROS_INFO("FLAGS_weight_target_speed : %f", FLAGS_weight_target_speed);
    ROS_INFO("FLAGS_weight_dist_travelled : %f", FLAGS_weight_dist_travelled);
    ROS_INFO("FLAGS_longitudinal_jerk_lower_bound : %f", FLAGS_longitudinal_jerk_lower_bound);
    ROS_INFO("FLAGS_longitudinal_jerk_upper_bound : %f", FLAGS_longitudinal_jerk_upper_bound);
    ROS_INFO("FLAGS_lon_collision_cost_std : %f", FLAGS_lon_collision_cost_std);
    ROS_INFO("FLAGS_lon_collision_yield_buffer : %f", FLAGS_lon_collision_yield_buffer);
    ROS_INFO("FLAGS_lon_collision_overtake_buffer : %f", FLAGS_lon_collision_overtake_buffer);
    std::cout << "\n";

    ROS_INFO("FLAGS_weight_lon_objective : %f", FLAGS_weight_lon_objective);
    ROS_INFO("FLAGS_weight_lon_jerk : %f", FLAGS_weight_lon_jerk);
    ROS_INFO("FLAGS_weight_lon_collision : %f", FLAGS_weight_lon_collision);
    ROS_INFO("FLAGS_weight_centripetal_acceleration : %f", FLAGS_weight_centripetal_acceleration);
    ROS_INFO("FLAGS_weight_lat_offset : %f", FLAGS_weight_lat_offset);
    ROS_INFO("FLAGS_weight_lat_comfort : %f", FLAGS_weight_lat_comfort);
    std::cout << "\n";

    ROS_INFO("FLAGS_speed_lon_decision_horizon : %f", FLAGS_speed_lon_decision_horizon);
    ROS_INFO("FLAGS_trajectory_space_resolution : %f", FLAGS_trajectory_space_resolution);
    ROS_INFO("FLAGS_lat_offset_bound : %f", FLAGS_lat_offset_bound);
    ROS_INFO("FLAGS_weight_opposite_side_offset : %f", FLAGS_weight_opposite_side_offset);
    ROS_INFO("FLAGS_weight_same_side_offset : %f", FLAGS_weight_same_side_offset);
    std::cout << "\n";

    ROS_INFO("FLAGS_speed_lower_bound: %f", FLAGS_speed_lower_bound);
    ROS_INFO("FLAGS_speed_upper_bound: %f", FLAGS_speed_upper_bound);
    ROS_INFO("FLAGS_longitudinal_acceleration_lower_bound: %f", FLAGS_longitudinal_acceleration_lower_bound);
    ROS_INFO("FLAGS_longitudinal_acceleration_upper_bound: %f", FLAGS_longitudinal_acceleration_upper_bound);
    ROS_INFO("FLAGS_comfort_acceleration_factor: %f", FLAGS_comfort_acceleration_factor);
    ROS_INFO("FLAGS_Max_curvature: %f", FLAGS_Max_curvature);
    ROS_INFO("FLAGS_kappa_bound: %f", FLAGS_kappa_bound);
    ROS_INFO("FLAGS_dkappa_bound: %f", FLAGS_dkappa_bound);
    ROS_INFO("FLAGS_lateral_acceleration_bound: %f", FLAGS_lateral_acceleration_bound);
    ROS_INFO("FLAGS_lateral_jerk_bound: %f", FLAGS_lateral_jerk_bound);
    std::cout << "\n";

    ROS_INFO("FLAGS_trajectory_time_length: %f", FLAGS_trajectory_time_length);
    ROS_INFO("FLAGS_polynomial_minimal_param: %f", FLAGS_polynomial_minimal_param);
    ROS_INFO("FLAGS_num_velocity_sample: %f", FLAGS_num_velocity_sample);
    ROS_INFO("FLAGS_min_velocity_sample_gap: %f", FLAGS_min_velocity_sample_gap);
    std::cout << "\n";

    ROS_INFO_STREAM("FLAGS_enable_osqp_debug: " << FLAGS_enable_osqp_debug);
    ROS_INFO("FLAGS_weight_lateral_offset: %f", FLAGS_weight_lateral_offset);
    ROS_INFO("FLAGS_weight_lateral_derivative: %f", FLAGS_weight_lateral_derivative);
    ROS_INFO("FLAGS_weight_lateral_second_order_derivative: %f", FLAGS_weight_lateral_second_order_derivative);
    ROS_INFO("FLAGS_lateral_third_order_derivative_max: %f", FLAGS_lateral_third_order_derivative_max);
    ROS_INFO("FLAGS_weight_lateral_obstacle_distance: %f", FLAGS_weight_lateral_obstacle_distance);
    ROS_INFO("FLAGS_default_delta_s_lateral_optimization: %f", FLAGS_default_delta_s_lateral_optimization);
    ROS_INFO("FLAGS_max_s_lateral_optimization: %f", FLAGS_max_s_lateral_optimization);
    ROS_INFO("FLAGS_bound_buffer: %f", FLAGS_bound_buffer);
    ROS_INFO("FLAGS_numerical_epsilon: %f", FLAGS_numerical_epsilon);
    ROS_INFO("FLAGS_nudge_buffer: %f", FLAGS_nudge_buffer);
    ROS_INFO("kSampleDistance: %f", kSampleDistance);
    ROS_INFO("FLAGS_lon_collision_buffer: %f", FLAGS_lon_collision_buffer);
    ROS_INFO("FLAGS_lat_collision_buffer: %f", FLAGS_lat_collision_buffer);
    std::cout << "\n";

    ROS_INFO("min_turn_radius: %f", min_turn_radius);
    ROS_INFO("max_acceleration: %f", max_acceleration);
    ROS_INFO("min_deceleration: %f", min_deceleration);
    ROS_INFO("max_deceleration: %f", max_deceleration);
    ROS_INFO("preferred_max_acceleration: %f", preferred_max_acceleration);
    ROS_INFO("preferred_min_deceleration: %f", preferred_min_deceleration);
    ROS_INFO("wheel_rolling_radius: %f", wheel_rolling_radius);
    ROS_INFO("brake_deadzone: %f", brake_deadzone);
    ROS_INFO("throttle_deadzone: %f", throttle_deadzone);
    std::cout << "\n";

    ROS_INFO("FLAGS_default_lon_buffer: %f", FLAGS_default_lon_buffer);
    ROS_INFO("FLAGS_time_min_density: %f", FLAGS_time_min_density);
    ROS_INFO("FLAGS_num_sample_follow_per_timestamp: %f", FLAGS_num_sample_follow_per_timestamp);
    std::cout << "\n";

    ROS_INFO("FLAGS_vehicle_width: %f", FLAGS_vehicle_width);
    ROS_INFO("FLAGS_vehicle_length: %f", FLAGS_vehicle_length);
    std::cout << "\n";

    ROS_INFO("front_edge_to_center: %f", front_edge_to_center);
    ROS_INFO("back_edge_to_center: %f", back_edge_to_center);
    ROS_INFO("left_edge_to_center: %f", left_edge_to_center);
    ROS_INFO("right_edge_to_center: %f", right_edge_to_center);
    std::cout << "\n";

    ROS_INFO("wheel_base: %f", wheel_base);
    ROS_INFO("max_steer_angle: %f", max_steer_angle);
    ROS_INFO("max_steer_angle_rate: %f", max_steer_angle_rate);
    ROS_INFO("steer_ratio: %f", steer_ratio);
    ROS_INFO("max_abs_speed_when_stopped: %f", max_abs_speed_when_stopped);
    std::cout << "\n";

    ROS_INFO("FLAGS_enable_sqp_solver: %d", FLAGS_enable_sqp_solver);
    ROS_INFO("default_active_set_eps_num: %f", default_active_set_eps_num);
    ROS_INFO("default_active_set_eps_den: %f", default_active_set_eps_den);
    ROS_INFO("default_active_set_eps_iter_ref: %f", default_active_set_eps_iter_ref);
    std::cout << "\n";

    ROS_INFO("default_qp_smoothing_eps_num: %f", default_qp_smoothing_eps_num);
    ROS_INFO("default_qp_smoothing_eps_den: %f", default_qp_smoothing_eps_den);
    ROS_INFO("default_qp_smoothing_eps_iter_ref: %f", default_qp_smoothing_eps_iter_ref);
    std::cout << "\n";

    ROS_INFO("default_enable_active_set_debug_info: %d", default_enable_active_set_debug_info);
    ROS_INFO("default_qp_iteration_num: %d", default_qp_iteration_num);
    std::cout << "\n";
